...
# sequence by once place, you may be tempted to think that we would get the same sequence 
...
# really recommend to any math enthusiast and for showing me this mathematical 
# curiosity too with his usual contagious passion :)]

def tribonacci(arr_, n):
    if n == 0:
        return []
    res = arr_
    a, b, c = arr_
...
def tribo_fast(arr_, n):
    res = arr_[:n]
    for _ in range(n-3): res.append(sum(res[-3:]))
    return res

print(tribonacci(list(map(int, input('Enter array: ').split())), int(input('Enter N: ')))) 