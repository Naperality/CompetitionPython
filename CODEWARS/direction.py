...
# "NORTH", "SOUTH", "WEST", "EAST". Clearly "NORTH" and "SOUTH" 
# are opposite, "WEST" and "EAST" too.

# Going to one direction and coming back the opposite direction right away 
# is a needless effort. Since this is the wild west, with dreadful weather and not much 
...
# better stay to the same place! So the task is to give to the man a simplified version of 
# the plan. A better plan in this case is simply:
...
# not directly opposite but they become directly opposite after the reduction of "EAST" and 
# "WEST" so the whole path is reducible to ["WEST", "WEST"].
...
def path_reduc(paths):
    ...
    res = []
    for dir in paths:
        if res and res[-1] == op.get(dir):
            res.pop()
        else:
            res.append(dir)
    return res

print(path_reduc(list(map(str, input('Enter Path: ').split()))))