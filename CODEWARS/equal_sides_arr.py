...
# Your function will return the index 3, because the sum of left side 
# of the index ({1,2,3}) and the sum of the right side of the index ({3,2,1}) both equal 6.

# Let's look at another one.
# You are given the array {1,100,50,-51,1,1}:
# Your function will return the index 1, because the sum of left 
...
def find_even(arr):
    if not arr:
        return -1
    total, left_s = sum(arr), 0
    for n in range(len(arr)):
        current = arr[n]
        right_s = total-left_s-current
        if right_s == left_s:
            return n
        left_s += current
    return -1
