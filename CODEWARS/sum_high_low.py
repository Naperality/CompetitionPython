...
# { 1, 1, 11, 2, 3 } => 6
# Input validation
# If an empty value ( null, None, Nothing, nil etc. ) is 
# given instead of an array, or the given array is an empty list 
# or a list with only 1 element, return 0.

def sum_high_low(arr):
    return sum(sorted(arr)[1:-1]) if arr else 0

print(sum_high_low(list(map(int, input('Enter Array: ').split()))))