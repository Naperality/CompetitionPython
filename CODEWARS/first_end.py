...
# two letters. beast and dish may contain hyphens and spaces, but these will not appear 
# at the beginning or end of the string. They will not contain numerals.

def feast(beast, dish):
    # return (beast[0],beast[len(beast)-1]) == (dish[0],dish[len(dish)-1])
    return (beast[0], beast[-1]) == (dish[0], dish[-1])

def feastv1(beast, dish):
    return beast.startswith(dish[0]) and beast.endswith(dish[-1])

print(feast(input('Enter Beast: '), input('Enter Dish: ')))
print(feastv1(input('Enter Beast: '), input('Enter Dish: ')))