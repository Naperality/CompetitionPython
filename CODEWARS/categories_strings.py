...
# Each book has a code of at least 3 characters. The 1st character of a code is a capital 
...
# find the total number of books in the bookseller's stocklist, with the category codes 
# in the list of categories. Note: the codes are in the same order in both lists.

# Return the result as a string described in the example below, or as a list of pairs 
# (Haskell/Clojure/Racket/Prolog).

# If any of the input lists is empty, return an empty string, or an empty array/list 
# (Clojure/Racket/Prolog).
...
def stock_list(stocklist, categories):
    res = {key:0 for key in categories}
