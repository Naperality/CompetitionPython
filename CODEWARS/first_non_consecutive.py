...
def first_non_consecutive(arr_):
    for i in range(len(arr_)):
        if i+1 < len(arr_) and arr_[i]+1 != arr_[i+1]:
            return arr_[i+1]
    return None

def first_non_consecutive_2(arr_):
    return next((arr_[i+1] for i in range(len(arr_) - 1) if arr_[i] + 1 != arr_[i+1]), None)

print(first_non_consecutive(list(map(int, input('Enter Array Numbers: ').split()))))